...
if __name__ == "__main__":
    ...
    candidates.write_parquet("data/candidates/test_rule_based_candidates.parquet")
    ...
    network_cand_top_20 = (
        train_log.join(network_cand_top_20, on="yad_no", how="inner")
        .select(["session_id", "candidate_yad_no"])
        .rename({"candidate_yad_no": "yad_no"})
        .with_columns(pl.lit(0, dtype=pl.UInt32).alias("count"))
        .with_columns(pl.lit(5).alias("priority"))
    ...
    print(next_seq)
    next_seq = (
        train_log.join(next_seq, left_on="yad_no", right_on="latest_yad_no", how="inner")
        .join(train_log_len, on="session_id", how="left")
        .filter(pl.col("seq_no") == pl.col("seq_count") - 1)
        .select(["session_id", "yad_no"])
        .with_columns(pl.lit(0, dtype=pl.UInt32).alias("count"))
    ...
    candidates.write_parquet("data/candidates/train_rule_based_candidates.parquet")
