...
def get_features(feature_list, df_yad, df_log, df_candidate, train_test="train", n_pivot=5):
    ...
    enumerated = df_log.sort(["session_id", "seq_no"]).with_columns(
    ...
    next_seq = pl.read_parquet(
        f"data/candidates/{train_test}_top10_candidates.parquet")
    df_candidate = (df_candidate.join(
    ...
    df_candidate = (df_candidate.join(
        next_seq, how="left", left_on=["yad_no_2", "yad_no_cand"], right_on=["latest_yad_no", "yad_no"]).with_columns(
        pl.col("count").cast(pl.Int16)).rename({"count": "next_seq_count_2_cand"})
    )
    ...
    for feature_name in tqdm(feature_name_list):
        ...
        if train_test == 'train':
            ...
            if 'latest_yad_no' in feature.columns:
            ...
            elif 'session_id' in feature.columns:
                df_candidate = df_candidate.join(feature, how='left', left_on=[
                    'session_id', 'yad_no_cand'], right_on=["session_id", "yad_no"])
                print(df_candidate.filter(pl.col("max_seq_no").is_null()))
            else:
                feature = feature.with_columns(pl.col("fold").cast(pl.Int64), )
                df_candidate = df_candidate.join(feature, how='left', left_on=[
    ...
    null_columns = set([
        col for col in df_candidate.columns if df_candidate[col].null_count() > 0])
    print(null_columns, )
...
def make_feature_and_label(candidate_train_tstamp, candidate_test_tstamp, featuer_list=None):
    ...
    # columns which has any null values
    null_columns = set([
    ...
    print(null_columns, null_columns_test)
